...
export default function Report_Monthly_patient() {
        ...
        if (!viewingYearly) {
            ...
            weeks.forEach((_, index) => {
                const weekData = filteredAppointments.filter(appointment => {
                    ...
                    const startOfWeek = new Date(date.getFullYear(), date.getMonth(), date.getDate() - date.getDay() + (index * 7));
    ...
    return (
        <div className="p-4 min-h-screen">
            <ReportMenu />
            <h2 className="text-2xl font-bold text-green-400">Patient Report</h2>
            ...
            <div className="pb-7 flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-3">
                ...
                {!viewingYearly ? (
                    <div className="flex space-x-4 items-center">
                        ...
                        {!isToday ? ( // Show these buttons only if not in 'Today' view
                            <>
                                ...
                                <button
                                    onClick={() => setMonth(new Date(new Date(month).setMonth(new Date(month).getMonth() - 1)).toISOString().slice(0, 7))}
                    ...
                    <div>
                        ...
                        {/* <h3 className="mt-4 text-lg">Yearly Report for {selectedYear}</h3> */}
                ...
                <button
                    onClick={() => {
                        ...
                        setIsToday(false); // Reset to false when switching to monthly view
                        if (viewingYearly) {
                            setMonth(new Date().toISOString().slice(0, 7)); // Reset month to current when switching to monthly
                        }
                    }}
                    className="mt-2 sm:mt-0 px-4 py-2 bg-yellow-500 text-white rounded hover:bg-yellow-600 transition duration-200"
            ...
            {isToday ? (
                <div>
                    <h3 className="text-lg font-semibold mt-4">Today's Appointments</h3>
                        ...
                        <tbody>
                            {filteredAppointments.map((appointment) => (
                                <tr key={appointment._id}>
                                    <td className="border border-gray-300 p-2">{appointment.patient.LastName} {appointment.patient.FirstName}</td>
                                    <td className={`border border-gray-300 p-2 font-bold ${appointment.status === 'Completed' ? 'text-green-400' : 'text-red-400 '}`}>
                                        {appointment.status}
                                    </td>
                ...
                <div>
                    ...
                    {isToday && (
                        ...
                        <div>
                            <h3 className="text-lg font-semibold mt-4">Appointment Summary</h3>
                            <table className="min-w-full border-collapse border border-gray-200 mt-2">
                                <thead>
                                    <tr>
                                        <th className="border border-gray-300 p-2">Patient Name</th>
